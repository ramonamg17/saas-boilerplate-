...
async def create_checkout_session(user: User, plan_key: str) -> str:
    ...
    session = stripe.checkout.Session.create(
        ...
        success_url=f"{settings.FRONTEND_URL}/dashboard/plan?success=1",
        cancel_url=f"{settings.FRONTEND_URL}/dashboard/plan?canceled=1",
...
async def _on_checkout_completed(data: dict, db: AsyncSession) -> None:
    from backend.core.email import send_subscription_confirmed
    from backend.plans import get_plan
...
async def _on_payment_failed(data: dict, db: AsyncSession) -> None:
    from backend.core.email import send_payment_failed
